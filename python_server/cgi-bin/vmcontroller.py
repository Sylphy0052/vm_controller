...
def p():
    ...
    ip_data_list = _get_dbinfo(_ipdata, sql)
    result = '''
[ipdata]<br>{0:^5}|{1:^15}|{2:^5}|{3:^5}|{4:^5}
<br>'''.format("id", "ipaddress", "type", "cpu", "mem")
    for row in ip_data_list:
        # print(row)
        result += '{0:^5}|{1:^15}|{2:^5}|{3:^5.1f}|{4:^5.1f}<br>'.format(row[0], row[1], row[2], row[3], row[4])
    line = "=" * 50
    result += '<br>{0}<br>'.format(line)

    sql = 'select id, ipadd, vmip, telnet_port from vmdata where ipadd <> -1'
    vm_data_list = _get_dbinfo(_vmdata, sql)
    result += '''
[vmdata]<br>{0:^5}|{1:^15}|{2:^15}|{3:^10}
<br>'''.format("id", "ipaddress", "vmipaddress", "telnetport")
    for row in vm_data_list:
        result += '{0:^5}|{1:^15}|{2:^15}|{3:^10}<br>'.format(row[0], row[1], row[2], row[3])
    line = "=" * 50
    result += '<br>{0}<br>'.format(line)

    sql = 'select * from clonedata'
    clone_data_list = _get_dbinfo(_clonedata, sql)
    result += '[clonedata]<br>{0:^5}|{1:^15}|{2:^15}<br>'.format("id", "from_ipaddress", "clone_ipaddress", "telnetport")
    for row in clone_data_list:
        result += '{0:^5}|{1:^15}|{2:^15}<br>'.format(row[0], row[1], row[2])
